...
vector<vector<int>> swapNodes(vector<vector<int>> indexes, vector<int> queries) {
    ...
    for (int k : queries) {
        ...
        function<void(int, int)> dfsSwap = [&](int node, int depth) {
            if (node == -1) return;
            ...
            dfsSwap(indexes[node - 1][0], depth + 1);
            dfsSwap(indexes[node - 1][1], depth + 1);
        };
        ...
        function<void(int)> dfsInorder = [&](int node) {
            if (node == -1) return;

            dfsInorder(indexes[node - 1][0]);
            inorder.push_back(node);
            dfsInorder(indexes[node - 1][1]);
        };
